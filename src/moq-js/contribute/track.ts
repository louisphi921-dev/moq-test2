...
export class Track {
	...
	constructor(media: MediaStreamTrack, config: BroadcastConfig) {
		...
		if (isAudioTrack(media)) {
			...
			throw new Error(`unknown track type: ${(media as any).kind}`)
		}
	}

	async #runAudio(track: MediaStreamTrack, config: AudioEncoderConfig) {
		const source = new (globalThis as any).MediaStreamTrackProcessor({ track })
	...
	async #runVideo(track: MediaStreamTrack, config: VideoEncoderConfig) {
		const source = new (globalThis as any).MediaStreamTrackProcessor({ track })
...
function isAudioTrack(track: MediaStreamTrack): track is MediaStreamTrack {
	return (track as any).kind === "audio"
}

function isVideoTrack(track: MediaStreamTrack): track is MediaStreamTrack {
	return (track as any).kind === "video"
}
